...
module TextHelper
  ...
  def truncate_html(text, max_length = 30, ellipsis = "...")
    ...
    ellipsis_length = Hpricot(ellipsis).inner_text.chars.count
    content_length = doc.inner_text.chars.count
...
module HpricotTruncator
  module NodeWithChildren
    def truncate(max_length)
      return self if inner_text.chars.count <= max_length
      truncated_node = self.dup
      truncated_node.children = []
      each_child do |node|
        remaining_length = max_length - truncated_node.inner_text.chars.count
